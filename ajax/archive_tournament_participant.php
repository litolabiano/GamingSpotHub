...
// 2. Perform Archive (update status to 'archived')
$stmt = $conn->prepare("UPDATE tournament_participants SET status = 'archived', removed_at = NOW() WHERE participant_id = ?");
$stmt->bind_param('i', $participant_id);

if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        $timestamp = date('M d, Y h:i A');
        $logDetails = "Archived Participant $pName from Tournament: $tName (#$tId) at $timestamp";
        
        $currentUser = getCurrentUser();
        logActivity($currentUser['user_id'], "Tournament Participant Archived", $logDetails);

        // 3. Notify the user if it's not a walk-in
        if ($targetUserId > 0) {
            $notifMsg = "Your entry for the tournament '$tName' has been archived by the admin.";
            $notifStmt = $conn->prepare("INSERT INTO notifications (user_id, type, message) VALUES (?, 'tournament_archive', ?)");
            $notifStmt->bind_param('is', $targetUserId, $notifMsg);
            $notifStmt->execute();
        }

        echo json_encode(['success' => true, 'message' => 'Participant archived successfully. Entry remains visible to user as "Archived".']);
    } else {
        echo json_encode(['success' => false, 'message' => 'No record updated. It might have been already archived.']);
    }
} else {
